...
str_p new_str(const char* string)
{
	...
	memcpy_s(p->buffer, p->size, string, p->size);
	return p;
}
...
{
	size_t new_size = strlen((*p)->buffer) + 1;
	str_p tmp = zero_str(new_size);
	errno_t err = memcpy_s(tmp->buffer, tmp->size, (*p)->buffer, new_size);
	free_str(*p);
	*p = tmp;

	return (*p)->size;
}
void rmv_eol(str_p* p)
{
	if ((*p)->buffer[strlen((*p)->buffer) - 1] == '\n')
	{
		if ((*p)->buffer[strlen((*p)->buffer) - 2] == '\r')
			(*p)->buffer[strlen((*p)->buffer) - 2] = '\0';	//windows
		else
			(*p)->buffer[strlen((*p)->buffer) - 1] = '\0';
	}
